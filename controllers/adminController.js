...
} = require("../utils/engineerRewardEngine");
...
exports.getAdminAnalyticsOverview = async (req, res) => {
  try {
    ...
    const courierCountResult =
      await sql`SELECT COUNT(*) FROM users WHERE role = 'courier'`;
...
exports.getCourierTrackingStatus = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch courier tracking status" });
  }
};
...
exports.updateCourierLocation = async (req, res) => {
  ...
  if (!deliveryId || !latitude || !longitude) {
    return res
      .status(400)
      .json({ error: "Missing deliveryId, latitude, or longitude" });
  }
...
exports.createProduct = async (req, res) => {
  try {
    const { name, description, price, unit, stock_quantity, producer_id } =
      req.body;
    const file = req.file;
...
exports.updateProduct = async (req, res) => {
  const { id } = req.params;
  const { name, description, unit, stock_quantity, weightKg, locations } =
    req.body;
      ...
      for (const loc of locations) {
        if (!loc.locationId || typeof loc.price !== "number") {
          return res.status(400).json({
            message: "Each location update must have locationId and price",
          });
        }
        // Validate location exists
        const [existingLoc] =
          await sql`SELECT 1 FROM locations WHERE id = ${loc.locationId}`;
        if (!existingLoc) {
          return res.status(400).json({
            message: `Invalid locationId: ${loc.locationId} does not exist`,
          });
        }

        // Upsert price (use 0 if transportCost is undefined)
        const transportCost =
          loc.transportCost !== undefined ? loc.transportCost : 0;
        await sql`
...
exports.createOrder = async (req, res) => {
  ...
  try {
    ...
    if ((!dropoff_lat || !dropoff_lng) && delivery_address) {
      try {
        ...
        if (coords) {
          ...
          console.log(
            `✅ Geocoded to lat: ${dropoff_lat}, lng: ${dropoff_lng}`,
          );
        } else {
          console.warn(
            "⚠️ Could not geocode delivery address — fallback to user address.",
          );
        }
      } catch (geoErr) {
    ...
    await sql.begin(async (tx) => {
      ...
      console.log("📊 Inserted order dropoff coordinates:", {
        dropoff_lat,
        dropoff_lng,
      });
      ...
      for (const item of order_items) {
        ...
        if (productData[0].stock_quantity < quantity)
          throw new Error(
            `Insufficient stock for product: ${productData[0].name}`,
          );

        console.log(
          `📉 Deducting stock: ${quantity} from product ${productData[0].name} (current stock=${productData[0].stock_quantity})`,
        );
        await tx`
        ...
        await tx`
          ...
          `;
        console.log(
          `✅ Added ${quantity} x ${productData[0].name} to order ${orderId}`,
        );
      }
...
exports.getOrdersByUser = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ success: true, data: Array.from(ordersMap.values()) });
  } catch (err) {
...
exports.assignDelivery = async (req, res) => {
  ...
  if (!order_id || !courier_id) {
    return res
      .status(400)
      .json({ message: "order_id and courier_id are required" });
  }

  try {
    // 1. Verify order exists and is pending
    const order =
      await sql`SELECT * FROM orders WHERE id = ${order_id} AND status = 'pending'`;
    ...
    const courier =
      await sql`SELECT * FROM couriers WHERE user_id = ${courier_id} AND is_available = true`;
    if (!courier || courier.length === 0) {
      return res
        .status(404)
        .json({ message: "Courier not found or not available" });
    }
    ...
    return res
      .status(200)
      .json({ message: "Courier assigned successfully", delivery });
  } catch (err) {
...
exports.updateCourierLocation = async (req, res) => {
  ...
  try {
    ...
    if (!order || order.courier_id !== courier_id) {
      return res
        .status(403)
        .json({ message: "Courier not assigned to this order" });
    }
    ...
    try {
      ...
      if (io) {
        io.to(`order:${order_id}`).emit("courier:location", {
          order_id,
          lat,
          lng,
        });
...
exports.getNearestCouriers = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch nearest couriers" });
  }
};
...
exports.updateOrderStatus = async (req, res) => {
  ...
  try {
    ...
    // pull user fcm token
    const userRow =
      await sql`SELECT id, full_name, fcm_token FROM users WHERE id = ${order.user_id}`;
...
exports.addLocation = async (req, res) => {
  try {
    ...
    const [existing] =
      await sql`SELECT * FROM locations WHERE address = ${address}`;
    if (existing) {
      return res
        .status(409)
        .json({ message: "Location with this address already exists" });
    }
    ...
    if (!lat || !lng) {
      try {
        ...
        if (response.data.results.length > 0) {
          ...
          return res.status(400).json({
            message:
              "Unable to geocode address. Please provide latitude and longitude manually.",
          });
        }
      } catch (geocodeErr) {
        ...
        return res
          .status(500)
          .json({ message: "Geocoding failed. Try manual lat/lng." });
      }
    }
...
exports.addProducer = async (req, res) => {
  try {
    ...
    if (!name || !location_id) {
      return res
        .status(400)
        .json({ message: "Producer name and location_id are required" });
    }

    // Validate location exists
    const [location] =
      await sql`SELECT id, latitude, longitude FROM locations WHERE id = ${location_id}`;
    if (!location) {
    ...
    const [existing] =
      await sql`SELECT * FROM producers WHERE name = ${name} AND location_id = ${location_id}`;
    if (existing) {
      return res.status(409).json({
        message: "Producer with this name already exists in this location",
      });
    }
    ...
    res
      .status(201)
      .json({ message: "Producer created successfully", producer });
  } catch (err) {
...
exports.addProduct = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({
        message:
          "Missing required fields (name, stock_quantity, unit, producer_id, category_slug, price, and at least one location)",
      });
    }
    ...
    if (!req.files || req.files.length < 2) {
      return res
        .status(400)
        .json({ message: "At least 2 image files are required" });
    }

    // Validate producer exists
    const [producer] =
      await sql`SELECT id FROM producers WHERE id = ${producer_id}`;
    if (!producer) {
    ...
    let category =
      await sql`SELECT id FROM categories WHERE slug = ${category_slug}`;
    if (!category || category.length === 0) {
      ...
      const existingByName =
        await sql`SELECT id, slug FROM categories WHERE name = ${categoryName}`;
    ...
    try {
      ...
      for (const loc of parsedLocations) {
        if (!loc.locationId) {
          return res
            .status(400)
            .json({ message: "Each location must have locationId" });
        }
      ...
      res.status(201).json({
        message: "Product added successfully with locations",
        product,
      });
    } catch (insertError) {
...
exports.calculateTransport = async (req, res) => {
  const {
    ...
    baseCost = 0,
    productId,
  } = req.body; // Added productId for context
  try {
    // Validate inputs
    if (!originLat || !originLng || !destLat || !destLng) {
      return res.status(400).json({
        error:
          "Missing required coordinates (originLat, originLng, destLat, destLng)",
      });
    }
    ...
    if (!element || element.status !== "OK") {
      return res
        .status(400)
        .json({ error: "No route found or invalid coordinates" });
    }
...
exports.validatePromoCode = async (req, res) => {
  ...
  try {
    ...
    if (promo.max_uses !== null && promo.uses_count >= promo.max_uses) {
      return res
        .status(400)
        .json({ message: "Promo code has been fully used" });
    }
...
exports.rateCourier = async (req, res) => {
  try {
    ...
    if (existingRating)
      return res
        .status(400)
        .json({ message: "Order already rated or skipped" });
...
exports.changePassword = async (req, res) => {
  ...
  try {
    ...
    if (!user.password) {
      ...
      return res.status(400).json({
        error: "Password not set for this user. Please reset your password.",
      });
    }
...
exports.getProducersByCategory = async (req, res) => {
  try {
    ...
    const [category] =
      await sql`SELECT id FROM categories WHERE id = ${categoryId}`;
    if (!category)
      return res
        .status(404)
    ...
    if (distanceSort && (!user_lat || !user_lng)) {
      return res.status(400).json({
        success: false,
        message: "user_lat and user_lng required for distance sorting",
      });
    }
    ...
    res.status(500).json({
      success: false,
      message: "Internal server error fetching producers",
    });
  }
};
...
exports.getProductsByProducer = async (req, res) => {
  try {
    ...
    if (!producer)
      return res
        .status(404)
        .json({ success: false, message: "Producer not found" });
...
async function createOrder(user_id, items) {
  ...
  for (const item of items) {
    const product =
      await sql`SELECT price FROM products WHERE id = ${item.product_id}`;
    if (!product.length)
  ...
  try {
    ...
    if (payment_type === "order") {
      if (!items?.length)
        return res
          .status(400)
          .json({ error: "Items are required for order payment" });
      if (!delivery_address)
    ...
    // Handle DELIVERY payments
      ...
      if (!order_id)
        return res
          .status(400)
          .json({ error: "order_id required for delivery payment" });

      const [existingOrder] =
        await sql`SELECT delivery_fee FROM orders WHERE id = ${order_id}`;
      if (!existingOrder)
      ...
      if (!totalAmount || totalAmount <= 0) {
        return res
          .status(400)
          .json({ error: "Delivery fee is not set for this order" });
      }
    }
...
exports.verifyPayment = async (req, res) => {
  try {
    ...
    if (newStatus === "completed" && payment.payment_type === "order") {
      ...
      await sql.begin(async (tx) => {
        ...
        const [newOrder] = await tx`
          INSERT INTO orders (
            user_id, status, delivery_address,
        ...
        await exports.createNotification({
          ...
          body:   `Your order #${newOrder.id} has been placed. Total: ₦${amountNumber.toLocaleString()}. You'll receive updates soon.`,
          data:   { order_id: newOrder.id },
        });

        // ── FIX 1: Notify ALL admins ──
        ...
        for (const admin of admins) {
          await exports.createNotification({
            ...
            body:   `Order #${newOrder.id} placed by ${safeFields.name || "a buyer"} for ₦${amountNumber.toLocaleString()}. ${items.length} item(s). Address: ${safeFields.delivery_address}`,
            data:   { order_id: newOrder.id, type: "new_order" },
          });
        }

        console.log(`✅ Product order #${newOrder.id} created. ${admins.length} admin(s) notified.`);
      });
    }
    ...
    // =========================================
      ...
      if (!items.length) {
        throw new Error("No items found for quote order creation");
      }
      ...
      await sql.begin(async (tx) => {
        ...
        for (const item of items) {
          ...
          await tx`
            ...
            VALUES (
              ${newOrder.id}, NULL, ${qty},
              ${unitPrice}, ${itemTotal}, ${itemName}
            )
          `;
        }

        // ── FIX 2: Mark the quote request as paid ──
        if (payment.quote_request_id) {
          await tx`
            UPDATE quote_requests
            SET status     = 'paid',
                updated_at = NOW()
            WHERE id = ${payment.quote_request_id}
          `;
          console.log(`✅ Quote #${payment.quote_request_id} marked as paid.`);
        }
        ...
        await exports.createNotification({
          ...
          body:   `Your quote order #${newOrder.id} has been placed. Total: ₦${amountNumber.toLocaleString()}. You'll receive updates soon.`,
          data:   { order_id: newOrder.id },
        });
        ...
        for (const admin of admins) {
          await exports.createNotification({
            userId: admin.id,
            title:  "🧾 New Quote Order Paid",
            body:   `Quote order #${newOrder.id} paid by ${safeFields.name || "a buyer"} for ₦${amountNumber.toLocaleString()}. ${items.length} item(s). Address: ${safeFields.delivery_address}`,
            data:   {
              order_id:        newOrder.id,
              quote_request_id: payment.quote_request_id,
              type:            "new_quote_order",
            },
          });
        }

        console.log(`✅ Quote order #${newOrder.id} created. ${admins.length} admin(s) notified.`);
      });
    }
...
exports.getCourierDashboard = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch courier dashboard" });
  }
};
...
exports.pickupOrder = async (req, res) => {
  try {
    ...
    // Map user -> courier
    const [courier] =
      await sql`SELECT id FROM couriers WHERE user_id = ${userId} LIMIT 1`;
    if (!courier)
      return res
        .status(404)
        .json({ success: false, message: "Courier not found for this user" });
    const courierId = courier.id;
    ...
    return res
      .status(500)
      .json({ success: false, message: "Failed to pick up order" });
  }
};
...
exports.deliverOrder = async (req, res) => {
  try {
    ...
    try {
      const rewardResult = await awardEngineerPointsForOrder(orderId);
      console.log("✅ Engineer reward result:", rewardResult);
    } catch (rewardError) {
      console.error("⚠️ Engineer reward failed:", rewardError.message);
    }
...
exports.getCourierStats = async (req, res) => {
  ...
  try {
    ...
    const points = completedOrders.reduce(
      (sum, o) => sum + (o.delivery_fee || 0) + (o.bonus || 0),
      0,
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch courier stats" });
  }
};
...
exports.getCourierRatingsSummary = async (req, res) => {
  try {
    ...
    const average = count
      ? ratings.reduce((sum, r) => sum + r.courier_rating, 0) / count
      : 0;
    ...
    res.json({
      success: true,
      summary: { count, average, breakdown, bonusPoints, tier },
    });
  } catch (err) {
    console.error(err);
    res
...
exports.getCourierReferralLink = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch referral link" });
  }
};
...
exports.markOrderDelivered = async (req, res) => {
  try {
    ...
    if (!courier)
      return res
        .status(403)
        .json({ success: false, message: "Courier account not found" });
    const courierId = courier.id;
    ...
    if (!delivery)
      return res
        .status(403)
        .json({ success: false, message: "Not authorized for this delivery" });
    ...
    try {
      const rewardResult = await awardEngineerPointsForOrder(order_id);
      console.log("✅ Engineer reward result:", rewardResult);
    } catch (rewardError) {
      console.error("⚠️ Engineer reward failed:", rewardError.message);
    }
...
exports.getMyDeliveries = async (req, res) => {
  try {
    ...
    const courierRow =
      await sql`SELECT id FROM couriers WHERE user_id = ${courierUserId} LIMIT 1`;
...
exports.courierDeliverOrder = async (req, res) => {
  try {
    ...
    if (!orderId || !courierId)
      return res
        .status(400)
        .json({ success: false, message: "OrderId and CourierId required" });
...
exports.getCourierDeliveryHistory = async (req, res) => {
  try {
    ...
    if (!courierId) {
      ...
      return res
        .status(401)
        .json({ success: false, message: "Invalid courier token" });
    }
    ...
    const deliveries = await sql`
      ...
      `;
    console.log(
      `[getCourierDeliveryHistory] fetched ${deliveries.length} deliveries`,
    );
    ...
    const deliveriesWithItems = await Promise.all(
      deliveries.map(async (d) => {
        const items = await sql`
          ...
          `;
        console.log(
          `[getCourierDeliveryHistory] order ${d.order_id} has ${items.length} items`,
        );
    ...
    console.log(
      "[getCourierDeliveryHistory] returning response with deliveries:",
      deliveriesWithItems.length,
    );
...
exports.getCourierReferralLink = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to get referral link" });
  }
};
...
exports.setFeaturedProducts = async (req, res) => {
  try {
    ...
    if (!productIds || !Array.isArray(productIds)) {
      return res
        .status(400)
        .json({ error: "productIds must be an array of UUIDs" });
    }
...
exports.deleteProducer = async (req, res) => {
  try {
    ...
    res.json({
      message: "Producer and associated products deleted successfully",
    });
  } catch (err) {
...
exports.createQuotePaymentLink = async (req, res) => {
  try {
    ...
    // ── FIX: Save quote_request_id so verifyPayment can update quote status ──
    const paymentId = uuidv4();
    await sql`
      INSERT INTO payments (
    ...
    // Mark quote as payment_initiated (not yet paid — verifyPayment sets it to 'paid')
    try {
      await sql`
        UPDATE quote_requests
