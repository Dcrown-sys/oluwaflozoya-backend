...
exports.searchProducts = async (req, res) => {
  ...
  exports.submitCourierKYC = [
    ...
    async (req, res) => {
      try {
        ...
        if (!firebaseUid) {
          return res.status(401).json({ success: false, message: 'Unauthorized: No Firebase UID found' });
        }
  
        // 🔍 Look up the user in your database by Firebase UID
        const userResult = await sql`
        ...
        const courierId = userResult[0].id; // ← your UUID
  
        const { full_name, phone, address, vehicle_type, vehicle_plate } = req.body;
  
        if (!full_name || !phone || !req.files?.selfie || !req.files?.document) {
          return res.status(400).json({ success: false, message: 'Missing required KYC fields' });
        }
  
        const selfie_url = `/uploads/${req.files.selfie[0].originalname}`;
        const document_url = `/uploads/${req.files.document[0].originalname}`;
  
        const result = await sql`
          INSERT INTO courier_kyc
          (courier_id, full_name, phone, address, vehicle_type, vehicle_plate, selfie_url, document_url, status, created_at)
          VALUES (${courierId}, ${full_name}, ${phone}, ${address}, ${vehicle_type}, ${vehicle_plate}, ${selfie_url}, ${document_url}, 'pending', NOW())
          RETURNING *;
        `;
  
        res.status(200).json({ success: true, message: 'KYC submitted successfully', data: result[0] });
  ...
  exports.verifyCourierKYC = async (req, res) => {
    try {
      const { courier_id } = req.params; // UUID of the courier from users.id
      const { status, notes } = req.body; // e.g. { status: 'approved', notes: 'Looks valid' }
  
      if (!courier_id || !status) {
        return res.status(400).json({ success: false, message: 'Missing required fields: courier_id or status' });
      }
      ...
      // 🔍 Ensure courier exists and has a pending KYC
      const existing = await sql`
        SELECT kyc.*, u.full_name AS courier_name, u.email AS courier_email, u.phone AS courier_phone
        FROM courier_kyc kyc
        JOIN users u ON u.id = kyc.courier_id
        WHERE kyc.courier_id = ${courier_id} AND kyc.status = 'pending';
      `;
  
      if (existing.length === 0) {
        return res.status(404).json({ success: false, message: 'No pending KYC found for this courier' });
      }
      ...
      // 💡 Optionally update the courier's main status too
      await sql`
        UPDATE users
      ...
      // 🎯 Fetch full updated info for response
      const updated = await sql`
        SELECT kyc.*, u.full_name AS courier_name, u.email AS courier_email, u.phone AS courier_phone
        FROM courier_kyc kyc
        JOIN users u ON u.id = kyc.courier_id
        WHERE kyc.courier_id = ${courier_id};
      `;
  
      res.status(200).json({
        success: true,
        message: `Courier KYC ${status} successfully`,
        data: updated[0],
      });
